...
# using the WSJ 20K standard language models
#
# We use the WSJ non-verbalized 20k closed word list. 
#
# Currently this LM isn't used elsewhere in the recipe.
...
grep -v "#" $WSJ0_DIR/WSJ0/LNG_MODL/VOCAB/WLIST20C.NVP >dict_temp
...
cat dict_temp2 >>dict_20k
rm -f dict_temp dict_temp2

# Decompress the WSJ standard language model and build the word network
gunzip -d -c $WSJ0_DIR/WSJ0/LNG_MODL/BASE_LM/BCB20CNP.Z >lm_temp
HBuild -A -T 1 -C $TRAIN_COMMON/configrawmit -n lm_temp -u '<UNK>' -s '<s>' '</s>' -z dict_20k wdnet_bigram_20k >hbuild.log
rm -f lm_temp
